    ...
    for(int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for(int batch = 0; batch < num_batches-1; ++batch) {
          if (rank == 0){
            ...
            // MPI_Allreduce(nn_gpu.dW1_h, nn_gpu.dW1_h_2, nn_gpu.n_hidden*nn_gpu.n_feats, MPI_FLOAT,
            //   MPI_SUM, MPI_COMM_WORLD);
            // MPI_Allreduce(nn_gpu.db1_h, nn_gpu.db1_h_2, nn_gpu.n_hidden*nn_gpu.n_batch, MPI_FLOAT,
            //   MPI_SUM, MPI_COMM_WORLD);
            // MPI_Allreduce(nn_gpu.dW2_h, nn_gpu.dW2_h_2, nn_gpu.n_classes*nn_gpu.n_hidden, MPI_FLOAT,
            //   MPI_SUM, MPI_COMM_WORLD);
            // MPI_Allreduce(nn_gpu.db2_h, nn_gpu.db2_h_2, nn_gpu.n_classes*nn_gpu.n_batch, MPI_FLOAT,
            //   MPI_SUM, MPI_COMM_WORLD);
...
void parallel_test(NeuralNetwork& nn, const arma::mat& X, const arma::mat& y,
    ...
    if (rank == 0){
      ...
      NeuralNetworkGPU nn_gpu(M,N_class,nn.H[1],batch_size, num_procs);
      nn_gpu.set_weights(nn.W[0], nn.b[0], nn.W[1], nn.b[1]);
